...
void main() {
  ...
  test(
    'an order with a product has customer, total and WhatsApp text',
    () async {
      final store = AppStore();
      ...
      final whatsapp = OrderShare.whatsappUri(order);
      expect(whatsapp, isNotNull);
      expect(whatsapp!.path, contains(order.customer.whatsappDigits));
  ...
  test(
    'closed orders leave the active list and stay in customer history',
    () async {
      final store = AppStore();
      final order = await seedTestOrder(store);
  ...
  test(
    'VAT is disabled by default and applies only to active orders',
    () async {
      final store = AppStore();
      final order = await seedTestOrder(store);
  ...
  test(
    ...
    () async {
      ...
      final uri = OrderShare.whatsappUri(order);
      expect(uri, isNotNull);
      expect(uri!.host, 'wa.me');
      expect(uri.path.replaceAll('/', ''), isEmpty);
      expect(uri.queryParameters['text'], contains(order.orderNumber));
      expect(uri.queryParameters['text'], contains(customer.name));
    },
  );
}
